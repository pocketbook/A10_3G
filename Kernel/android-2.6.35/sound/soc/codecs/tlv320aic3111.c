...
#define    AUDIO_CODEC_HEADSET_DRIVER_GPIO 	(161)
...
//&*&*&*BC1_110929: add microphone mute control function 
static int get_mic_mute (struct snd_kcontrol *kcontrol,	struct snd_ctl_elem_value *ucontrol);

static int set_mic_mute (struct snd_kcontrol *kcontrol,	struct snd_ctl_elem_value *ucontrol);	
//&*&*&*BC2_110929: add microphone mute control function 
/*
 ***************************************************************************** 
 * Global Variable
 ***************************************************************************** 
...
//&*&*&*BC1_110929: add microphone mute control function 
	SOC_SINGLE_EXT("Set mic Mute", 0x0, 0, 7, 0, get_mic_mute, set_mic_mute),
//&*&*&*BC1_110929: add microphone mute control function 
...
//	{DAC_CHN_REG, 0xD5},
	...
	{HPR_DRIVER, 0x6},//e
	/* Headphone drivers */
	{HPHONE_DRIVERS, 0xD4}, //0x44 0xC4
	/* HPL unmute and gain 0db */
	{LEFT_ANALOG_HPL, 0x92}, //0x80 0dB, 0x88 -4 dB, 0x92 -9dB
	/* HPR unmute and gain 0db */
	{RIGHT_ANALOG_HPR, 0x92}, //0x80 0dB, 0x88 -4 dB 0x92 -9dB
	{LEFT_ANALOG_SPL, 0x84}, //en plus 0x80 - 0x92 //for EP10
	...
	/* MICBIAS control */
	{MICBIAS_CTRL, 0x00}, //0x0A
	/* mic PGA unmuted */
	{MICPGA_VOL_CTRL, 0x5A},//0x76 0x5A 0x34
	/* IN1_L is selected for left P */
	{MICPGA_PIN_CFG,0x0C},//0x40
...
static int set_input_source (struct snd_kcontrol *kcontrol,	struct snd_ctl_elem_value *ucontrol){
//&*&*&*&*BC1_110916: fix cts ASR test fail issue
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	int inputSource = 0, data = 0;
	...
	aic3111_write(codec, MICPGA_VOL_CTRL, 0x34);
	
#ifdef CONFIG_MINI_DSP		

	data = aic3111_read(codec, MINIDSP_DAC_CONTROL);

	if(audio_headset_plugin) {
					
		if((data & 0x2) == 0x2)
		aic3111_write(codec, MINIDSP_DAC_CONTROL, 0x05); // Use buffer A
		
		aic3111_write(codec, LEFT_ANALOG_HPL, 0x88);
		aic3111_write(codec, RIGHT_ANALOG_HPR, 0x88);  
...
//&*&*&*BC2_110916:fix cts ASR test fail issue	
	return 0;
}
//&*&*&*BC2_110721: adjust the microphone gain for different input source
...
//&*&*&*BC1_110929: add microphone mute control function 
static int get_mic_mute (struct snd_kcontrol *kcontrol,	struct snd_ctl_elem_value *ucontrol){
	ucontrol->value.integer.value[0] = mic_mute;
	return 0;
}
...
static int set_mic_mute (struct snd_kcontrol *kcontrol,	struct snd_ctl_elem_value *ucontrol){
	...
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	mic_mute = (ucontrol->value.integer.value[0]);
	printk("mic_mute = %d\n", mic_mute);

	if(mic_mute)
		aic3111_write(codec, MICPGA_PIN_CFG, 0x00);	
	else	
		aic3111_write(codec, MICPGA_PIN_CFG, 0x0C);	
...
//&*&*&*BC1_110916:fix cts ASR test fail issue
	...
	if(audio_headset_plugin) {
		...
		if((data & 0x2) == 0x0)
		aic3111_write(codec, MINIDSP_DAC_CONTROL, 0x05); // Use buffer B
	...
	aic3111_write(codec, LEFT_ANALOG_HPL, 0x92);
	aic3111_write(codec, RIGHT_ANALOG_HPR, 0x92);  
...
static void aic3111_capture_io_control(struct snd_soc_codec *codec, int on)
{
	if(on)
	{
		aic3111_write(codec, MICBIAS_CTRL, 0x0A);
	...
	{
		aic3111_write(codec, MICBIAS_CTRL, 0x00);
...
//&*&*&*BC1_111220:fix bug(#2555) Entangled left and right audio channels 		
		if(audio_headset_plugin)
			aic3111_write(codec, DAC_CHN_REG, 0xD6);
		else		
			aic3111_write(codec, DAC_CHN_REG, 0xEA);
//&*&*&*BC2_111220:fix bug(#2555) Entangled left and right audio channels 			
//&*&*&*BC2_110725: fix the issue that the device has no sounds for some applications when the device resume				
		dac_reg = aic3111_read(codec, DAC_MUTE_CTRL_REG) & ~MUTE_ON;
		aic3111_write(codec, DAC_MUTE_CTRL_REG, dac_reg);	
...
static int aic3111_suspend(struct snd_soc_codec *codec)
{
	...
	aic3111_write(codec, DAC_CHN_REG, 0x02);
...
static int aic3111_headset_driver_switch(int on)
{
	...
	DBG( "aic3111_headset_driver_switch %d\n", on);
...
//&*&*&*BC1_111101:DUT have buzz during the music play duration and set DUT mute	
...
static ssize_t aic3111_aic_store(struct device * dev, struct device_attribute *attr, char * buf, size_t count)
{
	...
	if (error)
		return error;
	  printk("aic3111_aic_store  val =%d\n", val);
 
 	if(!audio_headset_plugin)
 	{
		if (val) {		
			aic3111_write(codec_work_var_glob, CLASS_D_SPK, 0xC6); // ON
		}
		else {
			if(fm_route_path != 2)
			aic3111_write(codec_work_var_glob, CLASS_D_SPK, 0x06); // OFF
		}
	}
	
	return count;
}
//&*&*&*BC1_111107: fix cts testAllFilesInSysAreNotWritable fail issue
static DEVICE_ATTR(aic, 0664, aic3111_aic_show, aic3111_aic_store);
//&*&*&*BC1_111107: fix cts testAllFilesInSysAreNotWritable fail issue
//&*&*&*BC2_111101:DUT have buzz during the music play duration and set DUT mute	
...
//&*&*&BC2_101001:implement headphone driver control
	...
	gpio_set_debounce(hph_detect_gpio, 310);
	hph_detect_irq = OMAP_GPIO_IRQ(hph_detect_gpio);
	
	aic3111->page_no = 0;

	DBG( "*** Configuring AIC3111 registers ***\n");
...
//&*&*&*BC1_111101:DUT have buzz during the music play duration and set DUT mute		
	device_create_file(&i2c->dev, &dev_attr_aic);
//&*&*&*BC2_111101:DUT have buzz during the music play duration and set DUT mute			
