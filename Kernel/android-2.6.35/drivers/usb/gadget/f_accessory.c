...
extern int twl4030_accessory_enable(int enable);
...
acc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	printk("[f_accessory.c]acc_function_bind dev: %p\n", dev);
	...
	printk("[f_accessory.c]%s speed %s: IN/%s, OUT/%s\n",
			gadget_is_dualspeed(c->cdev->gadget) ? "dual" : "full",
			f->name, dev->ep_in->name, dev->ep_out->name);
	...
	if (dev->function.disabled) {
		if (b_requestType == (USB_DIR_OUT | USB_TYPE_VENDOR)) {
			if (b_request == ACCESSORY_START) {
				...
				value = 0;
				printk("[Simon] ACCESSORY_START\n");
				//twl4030_accessory_enable(1);
			} 
			else if (b_request == ACCESSORY_SEND_STRING) 
			{
				...
				value = w_length;
				printk("[Simon] ACCESSORY_SEND_STRING index=%d \n",w_index);
			}
			...
			if (b_request == ACCESSORY_GET_PROTOCOL) {
				*((u16 *)cdev->req->buf) = PROTOCOL_VERSION;
				...
				printk("[Simon] ACCESSORY_GET_PROTOCOL\n");
			}
		}
	}
