...
{
	struct android_usb_function	*f;
	//&*&*&*AL1_20111026, To avoid RNDIS wrong,  bind accessory function in the last.
	struct android_usb_function	*f_accessory = NULL;
	char **functions = dev->functions;
	int i;
	...
	if(f_accessory)
		f_accessory->bind_config(dev->config);
	//&*&*&*AL2_20111026, , To avoid RNDIS wrong,  bind accessory function in the last.
...
void android_enable_function(struct usb_function *f, int enable)
{
	...
	{
		printk("[android.c]Enter %s - enable function, f->name:%s\n", __FUNCTION__, f->name);
		...
		if (!strcmp(f->name, "accessory") && enable) {
			...
			list_for_each_entry(func, &android_config_driver.functions, list) {
				if (strcmp(func->name, "accessory")
					...
					printk("[android.c]list_for_each_entry android_config_driver.functions, func->name:%s\n", func->name);
