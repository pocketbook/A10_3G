...
/*20111212, JimmySu modify G-sensor use bypass mode*/
...
static const struct adxl345_platform_data adxl345_default_init = {
	...
	.data_rate = /*7*/0x6, //3200HZ/(2^(15-x)), reg 0x2c, data rate bigger, more sensitive. /*0x6 -> 6,25Hz*/
...
void adxl345_work(struct work_struct *work)
{
...
#else
	adxl345_service_ev_fifo(ac);  /*20111212, JimmySu modify G-sensor use bypass mode*/
#endif
	...
	local_irq_enable();
#else
	schedule_delayed_work(&ac->work, ac->delaytime/10);
#endif
...
static ssize_t adxl345_delay_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val < 20)
		data_rate_change = 0xA | (adxl345_default_init.low_power_mode << 8);
	else if (val <40 ) 
		data_rate_change = 0x9 | (adxl345_default_init.low_power_mode << 8);
	else if (val < 80)
		data_rate_change = 0x8 | (adxl345_default_init.low_power_mode << 8);
	else if (val < 160)
		data_rate_change = 0x7 | (adxl345_default_init.low_power_mode << 8);
	else if (val < 320)
		data_rate_change = 0x6 | (adxl345_default_init.low_power_mode << 8);
	else if (val < 640)
		data_rate_change = 0x5 | (adxl345_default_init.low_power_mode << 8);
	else
		data_rate_change = 0x4 | (adxl345_default_init.low_power_mode << 8);

	AC_WRITE(ac, BW_RATE, data_rate_change);

	printk("[ADXL345 G-Sensor]:Enter %s, data rate=0x%x\n", __FUNCTION__, data_rate_change);
	printk("[ADXL345 G-Sensor]:Enter %s, delaytime=%d\n", __FUNCTION__, ac->delaytime);
    return count;
}
/*-----20111212, JimmySu modify the data rate with delay time*/
