...
static u16 current_fps = MT9D115_FPS_AUTO_7_5TO30;	//&*&*&*CJ_20110712 Implement FPS
...
printk("[mt9d115 Sensor]Enter %s Retrying I2C... %d\n", __FUNCTION__, retries); 
		retries++;
		mdelay(20);
		goto retry;
	}
...
//&*&*&*CJ1_20110712 Implement FPS
	if (sensor->fps >30)
	{
		current_fps = MT9D115_FPS_AUTO_7_5TO30;
	}
	else if (sensor->fps > 25)
	{
		current_fps = MT9D115_FPS_FIX30;
	}
	else if (sensor->fps > 20)
	{
		current_fps = MT9D115_FPS_FIX24;
	}
	else if (sensor->fps > 15)
	{
		current_fps = MT9D115_FPS_FIX20;
	}
	else if (sensor->fps > 10)
	{
		current_fps = MT9D115_FPS_FIX15;
	}
	else if (sensor->fps > 8)
	{
		current_fps = MT9D115_FPS_FIX10;
	}else
	{
		current_fps = MT9D115_FPS_AUTO_7_5TO30;
	}	
...
//&*&*&*CJ1_20110712 change for exposure & sharpness criteria
//	mt9d115_write_reg(client, MT9D115_MCU_DATA_0,	(data*2)+57, I2C_16BIT);
	mt9d115_write_reg(client, MT9D115_MCU_DATA_0,	(data*2)+66, I2C_16BIT); //&*&*&*CJ_20110802 change for exposure & sharpness criteria
//&*&*&*CJ2_20110712 change for exposure & sharpness criteria
...
//&*&*&*CJ1_20110712 Implement FPS
static int mt9d115sensor_set_fps(struct v4l2_int_device *s,u16 mode)
{
	int err = 0;
	struct mt9d115_sensor *sensor = s->priv;
	struct i2c_client *client = sensor->i2c_client;
		...
		switch (mode) 
		{
 		case MT9D115_FPS_FIX30:
			err = mt9d115_write_regs(client, fps_fix_30);
			break;

		case MT9D115_FPS_FIX24:
			err = mt9d115_write_regs(client, fps_fix_24);
			break;

		case MT9D115_FPS_FIX20:
			err = mt9d115_write_regs(client, fps_fix_20);
			break;

		case MT9D115_FPS_FIX15:
			err = mt9d115_write_regs(client, fps_fix_15);
			break;

		case MT9D115_FPS_FIX10:
		err = mt9d115_write_regs(client, fps_fix_10);
			break;

		case MT9D115_FPS_AUTO_7_5TO30:

		default:
			err = mt9d115_write_regs(client, fps_variable_7_5_to_30);
			break;
...
{
	u32 val;
	u32 i,j;	//&*&*&*CJ_20110728 for checking sensor command stability
	int err;
...
//&*&*&*CJ1_20110728 for checking sensor command stability			
			j=0;
enter_preview:
			mt9d115_write_reg(client, MT9D115_MCU_ADDRESS, MT9D115_MCU_SEQ_CMD, I2C_16BIT);
			...
			while( !((val & 0x3)==0x3) )
			{
				if( i>6)
				{
printk("[mt9d115 Sensor]Enter %s preveiw val = 0x%x enter_preview again\n", __FUNCTION__,val); 
				    j++;
				    if (j>=2)
				    {
				      err=-1;
printk("[mt9d115 Sensor]Enter %s preveiw val = 0x%x enter_preview fail err = %d\n", __FUNCTION__,val,err); 
				      break;
				    }  
				    goto enter_preview;
				}    
				mdelay(20);
				mt9d115_write_reg(client, MT9D115_MCU_ADDRESS, 0xA104, I2C_16BIT);
				mt9d115_read_reg(client, I2C_16BIT, MT9D115_MCU_DATA_0, &val);
				i++;
			}
//&*&*&*CJ2_20110728 for checking sensor command stability			
...
//&*&*&*CJ_20110728 for checking sensor command stability			
 			if(i<3)
			   mdelay(20);
...
enter_capture:
			...
			while( !((val & 0x7) == 0x7) )
			{
				if( i>6)
				{
printk("[mt9d115 Sensor]Enter %s capture val = 0x%x enter_capture again\n", __FUNCTION__,val); 
				    j++;
				    if (j>=2)
				    {
				      err=-1;
printk("[mt9d115 Sensor]Enter %s capture val = 0x%x enter_capture fail err = %d\n", __FUNCTION__,val,err); 
				      break;
				    }  
				    goto enter_capture;
				}    
...
static int mt9d115_configure(struct v4l2_int_device *s)
{
	...
	struct vcontrol *lvc = NULL;
 	u32 status;
 	unsigned int stream_on=0;	//&*&*&*CJ_20110728 for checking sensor command stability
...
//&*&*&*CJ1_20110610 for improve captrure performance
    ...
    {
//&*&*&*CJ1_20111013 modify for mipi stability improvement.
printk("[mt9d115 Sensor]Enter %s mipi off then on.\n", __FUNCTION__); 

//	 put the sensor core into standby
	err =mt9d115_read_reg(client, I2C_16BIT, 0x301a, &status);
	status = status & 0xfffb;
	mt9d115_write_reg(client, 0x301a, status, I2C_16BIT);
	...
	while (!(status & 0x02))
	{
	     ...
	     if (i >10)
	     {
printk("[mt9d115 Sensor]Enter %s core into standby status = 0x%x fail\n", __FUNCTION__,status); 
	        break;
	     }   
	     mdelay(10);
	     mt9d115_read_reg(client, I2C_16BIT, 0x303c, &status);
	}

//	 put the MIPI interface into standby
//&*&*&*CJ1_20110728 for checking sensor command stability
  	i=0;
	...
	mt9d115_write_reg(client, 0x3400, status, I2C_16BIT);
	  ...
	  while (!(status & 0x10))
	  {
	     ...
	     if (i >5)
	     {
printk("[mt9d115 Sensor]Enter %s stream_off status = 0x%x fail\n", __FUNCTION__,status); 
	        break;
	     }   
	    
	     mdelay(10);
	     err =mt9d115_read_reg(client, I2C_16BIT, 0x3402, &status);
...
stream_on:		
	  ...
	  status=0;
	  while (!(status & 0x20))
//	  while (((status & 0x31) != 0x20))
	  {
	     mdelay(10);
	     i++;
	     if (i >15 )
	     {
printk("[mt9d115 Sensor]Enter %s stream_on status = 0x%x fail\n", __FUNCTION__,status); 
	        break;
	     }   
	     err =mt9d115_read_reg(client, I2C_16BIT, 0x3402, &status);
	     if ( status & 0x1)
	     {
printk("[mt9d115 Sensor]Enter %s stream_on again\n", __FUNCTION__); 
		goto stream_on;	
	     }		
	 
	  }

//	take the sensor core out of standby
	  err =mt9d115_read_reg(client, I2C_16BIT, 0x301a, &status);
	  ...
	  while ((status & 0x10)==0x10)
	  {
	     if (i >15 )
	     {
printk("[mt9d115 Sensor]Enter %s core out of standby status = 0x%x fail\n", __FUNCTION__,status); 
	        break;
	     }   
	     mdelay(10);
	     err =mt9d115_read_reg(client, I2C_16BIT, 0x3402, &status);
...
//&*&*&*CJ2_20110610 for improve captrure performance
//&*&*&*CJ2_20110728 for checking sensor command stability
//&*&*&*CJ2_20111013 modify for mipi stability improvement.

	/* configure image size and pixel format */
	err =mt9d115_configure_frame(client, isize);
	if(err)
	{
printk("[mt9d115 Sensor]Enter %s mt9d115_configure_frame fail err=%d\n", __FUNCTION__,err); 
		return err;
	}	
...
//					sensor->v4l2_int_device, lvc);
	...
	mt9d115sensor_set_fps(sensor->v4l2_int_device,current_fps);	//&*&*&*CJ_20110712 Implement FPS
...
//&*&*&*CJ1_20110728 for checking sensor command stability
	...
	while ((status & 0xff) != 0)
	{
	        ...
	        mdelay(20);
		mt9d115_write_reg(client, MT9D115_MCU_ADDRESS, MT9D115_MCU_SEQ_CMD, I2C_16BIT);
	        err =mt9d115_read_reg(client, I2C_16BIT, MT9D115_MCU_DATA_0, &status);
		i++;
	        if (i>10)
	        {
printk("[mt9d115 Sensor]Enter %s refresh mode to run status = 0x%x  fail\n", __FUNCTION__,status); 
			break;
		}		
	}
	if(i<3)
...
//&*&*&*CJ1_20110728 for checking sensor command stability
	...
	while ((status & 0xff) != 0)
	{
		...
		i++;
	        if (i>10)
	        {
printk("[mt9d115 Sensor]Enter %s fresh mode to run status = 0x%x fail\n", __FUNCTION__,status); 
			break;
		}		
	}
	if(i<3)
...
//&*&*&*CJ2_20110728 for checking sensor command stability
	...
	err =mt9d115_read_reg(client, I2C_16BIT, 0x3408, &status);
printk("[mt9d115 Sensor]Enter %s sensor line byte = 0x%x \n", __FUNCTION__,status); 
//&*&*&*CJ2_20110728 for checking sensor command stability
...
retry_config:
			...
			if (err)
			{
				...
				if(i>=2)
				{
printk("[mt9d115 Sensor]Enter %s return err= %d\n", __FUNCTION__,err); 
				   return err;
				}   
				else
				{
printk("[mt9d115 Sensor]Enter %s retry_config\n", __FUNCTION__); 
				   goto retry_config;
				}   
			}	
		}
