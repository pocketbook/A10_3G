...
enum omap_vout_channels {
	OMAP_VIDEO1,
	OMAP_VIDEO2,
#if 0 //&*&*&*JJ1 20110811_page allocation failure
	//OMAP_VIDEO3,
#endif //&*&*&*JJ2 20110811_page allocation failure
};
...
static int fall_back_to_gfx; /*20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
...
static u32 vid2_static_vrfb_alloc=1;
static int debug;
...
static unsigned long omap_vout_alloc_buffer(u32 buf_size, u32 *phys_addr)
{
   ...
   printk("[%s]phys_addr=0x%x virt_addr=0x%x size=%d\n", __func__, *phys_addr, virt_addr, buf_size);
...
static void omap_vout_free_buffer(unsigned long virtaddr, u32 buf_size)
{
    ...
    printk(" [%s] addr: 0x%x, size: %d\n", __func__, (void*)virtaddr, buf_size);
...
#ifndef CONFIG_ARCH_OMAP4
    //printk(" [%s] addr: 0x%x, size: %d\n", __func__, (void*)virtaddr, size);
//	free_pages_exact((void *) virtaddr, size);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
	free_pages_exact((void *) virtaddr, buf_size);	//&*&*&*CJ_20110811_mod: follow TI MMS' solution for camera driver
#else
...
/*++++20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
	...
	//ISP cannot resize in MEM 2 MEM mode
	//pictures whos width is not even to 32
	if ((vout->pix.width*2) % 32) {
		fall_back_to_gfx = 1;
 		return;
	}
/*----20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
...
//	if (vout->pix.height * vout->pix.width == VID_MAX_WIDTH * 720) {
	if (vout->pix.height * vout->pix.width >= 480 * 360) {
...
#else
	return;
if(((vout->pix.width%16) == 0))
{
//	/* Check for media size */
//	if (/*((vout->pix.width * vout->pix.height) > (vout->win.w.width * vout->win.w.height))&&*/\
//			(vout->pix.width > vout->win.w.width)&&(vout->pix.height > vout->win.w.height)&&
//			(vout->pix.width > 800 /*1024*/)/*&& ((vout->pix.width%16) == 0)*/) 
//	{
//		goto enable_and_exit;
//	}

	/* Check for vertical scale */
	src_h = vout->pix.height;
	dst_h = vout->win.w.height;
	scale = (1024 * src_h)/dst_h;
	if (scale > 3072) {
		goto enable_and_exit;
	...
	u32 h_ratio,v_ratio,ratio,ratio_1,ratio_2,Ratio_V,Ratio_H;
	
	if(((vout->rotation==0)||(vout->rotation==2))&&(vout->pix.width > 1024))
	{
		
	//		if (vout->pix.width > 1024) //check 3-tap
			{
				...
				if(h_ratio*v_ratio > 3)				
					goto enable_and_exit;	
			}
	}	
	else/*((vout->rotation==1)||(vout->rotation==3))*/
	{
			ratio = 1;
			h_ratio = (1024 * 1024)/dst_w;
			v_ratio = (1024 * src_h)/dst_h;
			ratio_1 = (v_ratio*1024)/(2*h_ratio);
			
			if((2*1024 >= v_ratio)&&(v_ratio > 1*1024))
			{
				ratio = ratio_1;			
			}
			else if((4*1024 >= v_ratio)&&(v_ratio > 2*1024))
			{
				ratio_2 = ((v_ratio-2*1024)*1024)/(2*(h_ratio*1*1024));
...
//
			...
			h_ratio = (1024 * src_w)/dst_w;
			Ratio_H = h_ratio;
			
			if((Ratio_H > 3*1024)||(Ratio_V > 3*1024))
					goto enable_and_exit;	
...
{
/*+++++20111201, JimmySu modify the video height limitation*/
/*+++++20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
	int ifmt, bpp = 0;
...
//	int img_size = pix->height * pix->width;
	...
	pix->height = clamp(pix->height, (u32)VID_MIN_HEIGHT, (u32)VID_MAX_HEIGHT);
	pix->width = clamp(pix->width, (u32)VID_MIN_WIDTH, (u32)VID_MAX_WIDTH);
	pix->priv = 0;
	...
	if (ifmt == NUM_OUTPUT_FORMATS){
		pix->pixelformat = omap_formats[0].pixelformat;
	}
#endif
...
//		printk("%s vout: %p isprsz 0x%X win: %d/%d pix %d/%d\n", __func__,
//			   vout, vout->isprsz,
//			   vout->win.w.width, vout->win.w.height,
...
static int omap_vout_calculate_offset(struct omap_vout_device *vout, int idx)
{
	...
	switch (rotation) {
	case dss_rotation_90_degree:
		offset = vout->vrfb_context[0].yoffset * vout->vrfb_context[0].bytespp;
		temp_ps = ps / vr_ps;
		
		if (mirroring == 0) {
			*cropped_offset = offset + (line_length * temp_ps * cleft) + (crop->top * temp_ps);
		} else {
			*cropped_offset = offset + (line_length * temp_ps * cleft) + (crop->top * temp_ps) +
	...
	case dss_rotation_180_degree:
		offset = ((MAX_PIXELS_PER_LINE * vout->vrfb_context[0].yoffset * vout->vrfb_context[0].bytespp) +
				(vout->vrfb_context[0].xoffset * vout->vrfb_context[0].bytespp));

		if (mirroring == 0) {
			*cropped_offset = offset + (line_length * ps * ctop) + (cleft / vr_ps) * ps;
		} else {
			*cropped_offset = offset + (line_length * ps * ctop) + (cleft / vr_ps) * ps +
	...
	case dss_rotation_270_degree:
		offset = MAX_PIXELS_PER_LINE * vout->vrfb_context[0].xoffset * vout->vrfb_context[0].bytespp;
		temp_ps = ps / vr_ps;

		if (mirroring == 0) {
			*cropped_offset = offset + line_length * temp_ps * crop->left + ctop * ps;
		} else {
			*cropped_offset = offset + line_length * temp_ps * crop->left + ctop * ps +
								(line_length * ((crop->width / vr_ps) - 1) * ps);
		}
	...
	case dss_rotation_0_degree:
		if (mirroring == 0) {
			*cropped_offset = (line_length * ps) * crop->top + (crop->left / vr_ps) * ps;
		} else {
			*cropped_offset = (line_length * ps) * crop->top + (crop->left / vr_ps) * ps +
								(line_length * (crop->height - 1) * ps);
		}
	...
	default:
		*cropped_offset = (line_length * ps * crop->top) / vr_ps + (crop->left * ps) / vr_ps +
	...
	if (OMAP_DSS_COLOR_NV12 == vout->dss_mode) {
		*cropped_offset = tiler_stride(addr) * crop->top + crop->left;
		uv_addr = tiler_get_natural_addr(vout->queued_buf_uv_addr[idx]);
		
		/* :TODO: only allow even crops for NV12 */
		*cropped_uv_offset = tiler_stride(uv_addr) * (crop->top >> 1) + (crop->left & ~1);
	} else {
		*cropped_offset = tiler_stride(addr) * crop->top + crop->left * ps;
	}
#endif

    //printk("%s idx: %d Offset: %d\n",  __func__, idx, *cropped_offset);
...
/*+++++20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
	...
	nextI = vout->next_frm->i;

	addr = (u32) vout->queued_buf_addr[nextI] +
				 vout->cropped_offset[nextI];
			
	uv_addr = (u32) vout->queued_buf_uv_addr[nextI] +
					vout->cropped_uv_offset[nextI];
...
#ifdef CONFIG_EP10_PANEL_OPTIMIZATION
#include <plat/sdrc.h>
static int vrfb_channel;
#endif
//&*&*&*JJ2_20111102 EP5A panel for Video optimization
			    ...
			    enum v4l2_field field)
{
	...
	struct omap_vout_device *vout = q->priv_data;
//&*&*&*JJ1_20111102 EP10 panel for Video optimization
#ifdef CONFIG_EP5A_PANEL_OPTIMIZATION
	struct omapvideo_info *ovid = &vout->vid_info;
#endif
//&*&*&*JJ2_20111102 EP5A panel for Video optimization
...
//&*&*&*JJ1_20111102 EP10 panel for Video optimization
	...
	if((rotation==0)||(rotation==2))//0 or 180
	{
		if(vrfb_channel==0)
				vrfb_channel=1;
		else if(vrfb_channel==1)
				vrfb_channel=2;
		else
				vrfb_channel=0;			
...
//&*&*&*JJ1_20111102 EP10 panel for Video optimization
		...
		if (ret) {
			 ...
			 * from this array will be used to configure DSS */
			vout->queued_buf_addr[vb->i] = (u8 *)
				vout->vrfb_context[vb->i].paddr[r_dir];
		}
#endif
//&*&*&*JJ2_20111102 EP10 panel for Video optimization
		return ret;
...
//&*&*&*JJ1_20111102 EP10 panel for Video optimization
if((rotation==1)||(rotation==3)||(rotation==2)||(rotation==0))
{
#ifndef CONFIG_EP10_PANEL_OPTIMIZATION
	...
	omap_set_dma_dest_params(tx->dma_ch, 0, OMAP_DMA_AMODE_DOUBLE_IDX,
			vout->vrfb_context[vrfb_channel].paddr[w_dir], dest_element_index,
			dest_frame_index);
#endif
//&*&*&*JJ2_20111102 EP10 panel for Video optimization
...
#else
	vout->queued_buf_addr[vb->i] = (u8 *)
		vout->vrfb_context[vrfb_channel].paddr[r_dir];
}
else
{
...
//	spin_lock_init(&vout->vbq_lock);//&*&*&*JJ_20110420
...
//&*&*&*JJ1_20111102 EP10 panel for Video optimization
#ifdef CONFIG_EP10_PANEL_OPTIMIZATION
	if(vout->rotation != rotation_old)
		skip_video_frame_count();
		
	rotation_old = vout->rotation;
#endif
//&*&*&*JJ2_20111102 EP10 panel for Video optimization
	if (rotate_90_or_270(vout)) {
...
// 	printk(" [%s] userspace supplied data\nvout\n\tpix w/h: %d/%d\n\twin: w/h: %d/%d\n\trot: %d\n",
// 		   __func__, vout->pix.width, vout->pix.height,
// 		   vout->win.w.width, vout->win.w.height,
// 		   vout->rotation);
...
/*++++ 20110825, Jimmy Su add TI patch, fix video shift while playing*/
...
//		vrfb_configured = 1;
//	}
	static struct v4l2_window oldw;
	static struct v4l2_pix_format oldpix;

	if (oldpix.width != vout->pix.width ||
		oldpix.height != vout->pix.height ||
		
		oldw.w.width != vout->win.w.width ||
		oldw.w.height != vout->win.w.height ||
...
static int vidioc_streamon(struct file *file, void *fh, enum v4l2_buf_type i)
{
	...
	ret = omapvid_init(vout, addr, uv_addr);
	if (ret)
		v4l2_err(&vout->vid_dev->v4l2_dev, "failed to set overlay info\n");
...
finish:
//	INIT_LIST_HEAD(&vout->dma_queue);//&*&*&*JJ_20110420
...
//&*&*&*JJ1_20110831 for video don't used ISPRZR on 1st time.
	pix->width = 800;
	pix->height = 600;
//&*&*&*JJ2_20110831 for video don't used ISPRZR on 1st time.
...
#else
	int static_vrfb_allocation = 0, vrfb_num_bufs = 3;//VRFB_NUM_BUFS; //&*&*&*JJ++
#endif
#endif
...
#else
if(vid_num == 0)		//video 1 memory config
{
	for (i = 0; i < numbuffers; i++) {
		vout->buf_virt_addr[i] =
			omap_vout_alloc_buffer(vout->buffer_size,
...
else if(vid_num == 1)		//video 2 memory config
{
	for (i = 0; i < numbuffers; i++) {
		vid_dev->vouts[1]->buf_phy_addr[i]  = vid_dev->vouts[0]->buf_phy_addr[i];
		vid_dev->vouts[1]->buf_virt_addr[i] =	vid_dev->vouts[0]->buf_virt_addr[i];
		vout->cropped_offset[i] = 0;
	}
}
#endif
...
{
	for (i = 0; i < VRFB_NUM_BUFS; i++) {
		vid_dev->vouts[1]->smsshado_phy_addr[i]  = vid_dev->vouts[0]->smsshado_phy_addr[i];
		vid_dev->vouts[1]->smsshado_virt_addr[i] =	vid_dev->vouts[0]->smsshado_virt_addr[i];
	}
}
