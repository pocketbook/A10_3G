...
/*	dma_addr_t tmp_buf;
	size_t tmp_buf_size;*/ /*20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
	int status;
...
{
	if (dev_ctx.opened) {
		dev_err(NULL, "%s device is already opened\n", __func__);
		return -EBUSY;
	}

	dev_ctx.isp = isp_get();
	if (!dev_ctx.isp) {
		dev_err(NULL, "%s can't get ISP resource\n", __func__);
		return -EACCES;
	}
...
void ispdss_put_resource()
{
	...
	for (i = 0; i < dev_ctx.num_video_buffers; ++i) {
		if (dev_ctx.out_buf_virt_addr[i]) {
			iommu_kunmap(isp_res->iommu, dev_ctx.out_buf_virt_addr[i]);
			dev_ctx.out_buf_virt_addr[i] = 0;
		}
		
		if (dev_ctx.in_buf_virt_addr[i]) {
			iommu_kunmap(isp_res->iommu, dev_ctx.in_buf_virt_addr[i]);
...
/*++++20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
	...
	if (output_buffer_index >= dev_ctx.num_video_buffers) {
		dev_err(dev_ctx.isp,
				"ouput buffer index is out of range %d",
				output_buffer_index);
		return -EINVAL;
	}
	...
	out_buff = dev_ctx.out_buf_virt_addr[output_buffer_index];
	
	if (!out_buff) {
		output_size = pipe->out.image.height * out_off;

		out_buff = iommu_kmap(isp->iommu, 0,
							  out_phy_add, output_size,
							  IOMMU_FLAG);

		if (IS_ERR_VALUE(out_buff)) {
			dev_err(dev_ctx.isp, "Mapping of output buffer failed for %d\n",
					output_buffer_index);
			return -ENOMEM;
		}
		
		dev_ctx.out_buf_virt_addr[output_buffer_index] = out_buff;
	}

	in_buff = dev_ctx.in_buf_virt_addr[input_buffer_index];
	
	if (!in_buff) {
		in_buff = iommu_kmap(isp->iommu, 0,
							in_phy_add, in_off,
							IOMMU_FLAG);
		
		if (IS_ERR_VALUE(in_buff)) {
			dev_err(dev_ctx.isp, "Mapping of input buffer failed for %d \n",
					input_buffer_index);
			return -ENOMEM;
		}
		
		dev_ctx.in_buf_virt_addr[input_buffer_index] = in_buff;
	}
	...
	if (ispresizer_set_inaddr(isp_res, in_buff, pipe) != 0)
		return -EINVAL;

	if (ispresizer_set_out_offset(isp_res, out_off) != 0)
		return -EINVAL;

	if (ispresizer_set_outaddr(isp_res, out_buff) != 0)
		return -EINVAL;
