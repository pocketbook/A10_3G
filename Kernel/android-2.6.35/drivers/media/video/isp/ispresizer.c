...
/*++++20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
	if (pipe->out.image.width < MIN_OUT_WIDTH ||
		pipe->out.image.height < MIN_OUT_HEIGHT ||
		pipe->in.image.height < MIN_IN_HEIGHT ||
		pipe->in.image.width < MIN_IN_WIDTH)
			return -EINVAL;

/*----20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/

	pipe->out.image.height= min_t(u32, pipe->out.image.height, MAX_OUT_HEIGHT);
...
/*----20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
	...
	if (rsz < MIN_RESIZE_VALUE || rsz > MAX_RESIZE_VALUE) {
		rsz = clamp_t(s16, rsz, MIN_RESIZE_VALUE, MAX_RESIZE_VALUE);
		pipe->out.image.width = pipe->in.crop.width * RESIZECONSTANT / rsz;

		if (rsz == MIN_RESIZE_VALUE)
			pipe->out.image.width &= ~(OUT_WIDTH_ALIGN_UP - 1);
		else
			pipe->out.image.width = ALIGN(pipe->out.image.width, OUT_WIDTH_ALIGN_UP);
...
/*+++++20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
	if (h_rsz < MIN_RESIZE_VALUE || h_rsz > MAX_RESIZE_VALUE) {
		...
		pipe->in.crop.width = pipe->out.image.width * v_rsz /
				       RESIZECONSTANT;
	}
/*----20111007, Jimmy Su add, TI FIFO_UNDERFLOW patch*/
