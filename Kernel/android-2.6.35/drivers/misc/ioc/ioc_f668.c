...
{
	ioc_reset();
	return sprintf(buf, "IOC RESET OK\n");
}
...
static int iocf668_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	gpio_direction_output(UP_IOC_LEVEL_GPIO, 0);
...
static int iocf668_resume(struct i2c_client *client)
{
	...
	gpio_direction_output(UP_IOC_LEVEL_GPIO, 1);
...
static void iocf668_shutdown(struct i2c_client *client)
{
	printk("iocf668 shutdown\n");
	gpio_set_value(UP_IOC_LEVEL_GPIO, 0);
	gpio_set_value(IOC_RST_GPIO, 1);
	gpio_set_value(IOC_POWER_GPIO, 0);
