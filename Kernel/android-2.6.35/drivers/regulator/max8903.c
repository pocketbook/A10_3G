...
//&*&*&*BC1_110726: fix issue that cpu can not run 300 Mhz issue
...
#define CHARGING_FULL_THRESHOLD 100
#define BATTERY_HOT_THRESHOLD 45
#define BATTERY_COLD_THRESHOLD -5
#define BATTERY_REMOVE_THRESHOLD -31
...
extern int twl4030_cable_DP_DM_status(void);
#endif
...
extern int max17043_get_soc(void);
extern void max17043_set_charging_full_state(int state);
#else
...
//&*&*&*HC1_111109_Mod, only usb case needes to require high throughputs
//	if(!cable_state)
	if((!cable_state) && (max8903->supply == MAX8903_USB_CABLE_SUPPLY))
	omap_pm_set_min_mpu_freq(max8903->dev, 800000000);
	else
	omap_pm_set_min_mpu_freq(max8903->dev, -1);
//&*&*&*HC2_111109_Mod, only usb case needes to require high throughput

	printk("%s\n", __FUNCTION__);

}
//&*&*&*BC2_110726: fix issue that cpu can not run 300 Mhz issue
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CAPACITY:
			if(max8903->charging_state == MAX8903_CHARGING_STATUS_FULL)
				...
				val->intval = max8903->battery_soc;
			break;
		default:
			ret = -EINVAL;
			break;
...
static int max8903_get_supplies(struct max8903 *max8903)
{
	...
	if(supplies == MAX8903_VBUS_VALID_SUPPLY)
	{
		dp_dm_status = twl4030_cable_DP_DM_status();
		if(dp_dm_status == 1)
		{
			supplies = MAX8903_USB_CABLE_SUPPLY;
			status = schedule_delayed_work(&max8903->delay_work_reboot_state, 5*HZ);
		}
		else if(dp_dm_status == 0)
			supplies = MAX8903_USB_CABLE_SUPPLY;
		else if(dp_dm_status == 3)
			supplies = MAX8903_AC_CABLE_SUPPLY;
	}
...
{
#ifdef CONFIG_BATTERY_MAX17043
	max8903->battery_soc = max17043_get_soc();
#else
	...
	max8903->temperature = max17043_get_temperature();/*max8903_get_temperature();*/
...
#if 0
		//&*&*&*AL1_20101116: disable modem to avoid h/w damage
		gpio_set_value(S3C64XX_GPP(9), 0);
        s3c_gpio_cfgpin(S3C64XX_GPP(9), S3C64XX_GPP_OUTPUT(9));
        s3c_gpio_setpull(S3C64XX_GPP(9), S3C_GPIO_PULL_DOWN);
		//&*&*&*AL2_20101116: disable modem to avoid h/w damage
#endif			
...
#if 0   /* EP10 haven't any device connect on packet+ sied. */
		if(!regulator_is_enabled(max8903->max8903_regulator))
		{
            regulator_enable(max8903->max8903_regulator);
		}
		//&*&*&*AL2_20101116: enable charger and disable irq for battery remove
#endif				
		...
		case MAX8903_CHARGING_STATUS_CONDITION:
...
#endif
			{
				...
				else
				{
					...
					{
						max8903->full_recharging_en = 1;
						printk("[max8903 charger] Enter %s, Charger is determining FULL, but capacity is not 100% ....\n", __FUNCTION__);
					}
					else
					//&*&*&*HC1_20101221: Modify to prevent the wrong state transition during cable in/out event change
					{	
						printk("[max8903 charger] Enter %s, wrong state on charging complete, maybe charging fault ....\n", __FUNCTION__);
...
#ifdef CONFIG_BATTERY_MAX17043
	max17043_set_charging_full_state(max8903->full_recharging_en);
#endif
}
...
static void max8903_charger_handler(struct max8903 *max8903, int irq, void *data)
{
	...
	switch (irq) 
	{
		case MAX8903_PIN_CHARHEn:
			...
			*/
			#ifdef CONFIG_REGULATOR_MAX8903_CABLE_TYPE_DETECTION
			if(max8903->has_usb_configuration == 0 && max8903->supply == MAX8903_USB_CABLE_SUPPLY)
		...
		case MAX8903_PIN_DCOKn:
			...
			schedule_delayed_work(&max8903->delay_work_reboot_state, 5*HZ);
...
static irqreturn_t max8903_irq_handler(int irq, void *data)
{
	...
	switch (irq)
	{
		case 309:
			...
			schedule_delayed_work(&max8903->irq_delay_work_charge_state, 100);
			break;
#ifdef MAX8903_FULL_FUNCTION_SUPPORT
		case 203:
