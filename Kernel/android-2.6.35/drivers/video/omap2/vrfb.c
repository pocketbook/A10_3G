...
#else
#define VRFB_PAGE_WIDTH_EXP	6 /* Assuming SDRAM pagesize= 1024 */
#define VRFB_PAGE_HEIGHT_EXP	6 /* 1024 = 2^5 * 2^5 */
#endif
...
//	printk("\n\tpaddr: %lu w: %d h: %d bpp: %d yuv: %d rot: %d\n",
//		   paddr, width, height, bytespp, yuv_mode, rotation);
	...
	DBG("\tpixel_size_exp: %d ", pixel_size_exp);
	/* VDMA Optimization */
...
//&*&*&*JJ1_20111102 EP10 panel for Video optimization
	...
	DBG("\tvrfb w %u, h %u size: %d control: %d offsets x: %d y: %d\n",
		   vrfb_width, vrfb_height, size, control,
		   vrfb_width - width, vrfb_height - height);
	
	omap2_sms_write_rot_physical_ba(paddr, ctx);
#else
	if((ctx==0)||(ctx==1)||(ctx==2)||(ctx==6)||(ctx==7)||(ctx==8))
{
	size  = vrfb_width << SMS_IMAGEWIDTH_OFFSET;
	size |= vrfb_height << SMS_IMAGEHEIGHT_OFFSET;
}
...
{
	size  = vrfb_height  << SMS_IMAGEWIDTH_OFFSET;
	size |= vrfb_width   << SMS_IMAGEHEIGHT_OFFSET;
}
	...
	{
			vrfb_hw_context[ctx].physical_ba = paddr;
			vrfb_hw_context[ctx+3].physical_ba = paddr;
	}
	...
	{
			omap2_sms_write_rot_physical_ba(paddr, ctx);
			omap2_sms_write_rot_physical_ba(paddr, ctx+3);
	}
#endif
//&*&*&*JJ2_20111102 EP10 panel for Video optimization
