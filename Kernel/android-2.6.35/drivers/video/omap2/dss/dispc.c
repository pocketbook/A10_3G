...
void dispc_setup_plane_fifo(enum omap_plane plane, u32 low, u32 high)
{
	...
	if (REG_GET(ftrs_reg[plane], 11, 0) != low ||
		REG_GET(ftrs_reg[plane], 27, 16) != high){
		printk("fifo(%d) low/high old %u/%u, new %u/%u\n",
			plane,
...
//&*&*&*JJ1_20111102 EP10 panel for Video optimization
#ifdef CONFIG_EP10_PANEL_OPTIMIZATION
int skip_count=0;

void skip_video_frame_count(void)
{
		skip_count=5;
}
EXPORT_SYMBOL(skip_video_frame_count);
#endif
//&*&*&*JJ2_20111102 EP10 panel for Video optimization
...
//&*&*&*JJ1_20111102 EP10 panel for Video optimization
#ifndef CONFIG_EP10_PANEL_OPTIMIZATION
			...
			u16 height, u16 out_width, u16 out_height)
{
   ...
   if (cpu_is_omap44xx())
	...
	/* FIXME venc pclk? */
    //DSSERR("dispc_pclk_rate(channel) * vf * hf = %lu * %u * %u, channel=%d \n",
    //        dispc_pclk_rate(channel), vf, hf, channel);
...
{
//&*&*&*JJ1_20111102 EP5A panel for Video optimization
#ifdef CONFIG_EP10_PANEL_OPTIMIZATION
width/=2;
height/=2;
out_width/=2;
...
loop:
        ...
        if (((x == max_x_decim) && (reason & WIDTH_MAXDOWNSCALE_REACHED)) ||
			...
			DSSERR("failed to set up scaling, "
                    ...
                    "x=%d, max_x_decim=%d, y=%d, max_y_decim=%d\n",
                    fclk, dispc_fclk_rate(), x, max_x_decim, y, max_y_decim);
			return -EINVAL;
		}
...
#else
//&*&*&*JJ+++  //BA addr
	static int Notused_vrfb = 0;
	
	if (plane != OMAP_DSS_GFX) 
	{
//		if(1)
		//printk("[JJJXX](%s)(%d):width=%d \n",__FUNCTION__,__LINE__,width);
		if((width>1024)||((((height*1024)/out_height)>2048)&&(out_width>1000)))
		{			
			two_5_tap_enable = 1;
			
			if((rotation==OMAP_DSS_ROT_0)||(rotation==OMAP_DSS_ROT_180))
			{
				_dispc_set_plane_ba0(OMAP_DSS_VIDEO1, paddr + offset0 + (0));
				_dispc_set_plane_ba1(OMAP_DSS_VIDEO1, paddr + offset1 + (0));
				_dispc_set_plane_ba0(OMAP_DSS_VIDEO2, paddr + offset0 + (width) - ((width)%4));
				_dispc_set_plane_ba1(OMAP_DSS_VIDEO2, paddr + offset1 + (width) - ((width)%4));
			}
			else// if((rotation==OMAP_DSS_ROT_90)||(rotation==OMAP_DSS_ROT_270)||(rotation==OMAP_DSS_ROT_180))
			{
				_dispc_set_plane_ba0(OMAP_DSS_VIDEO1, paddr + offset0 + (0));
				_dispc_set_plane_ba1(OMAP_DSS_VIDEO1, paddr + offset1 + (0));
				_dispc_set_plane_ba0(OMAP_DSS_VIDEO2, paddr + offset0 + (width*2) - ((width)%4)*2);
				_dispc_set_plane_ba1(OMAP_DSS_VIDEO2, paddr + offset1 + (width*2) - ((width)%4)*2);
			}
	...
	if (plane != OMAP_DSS_GFX) 
	{
		if(rotation==OMAP_DSS_ROT_0)
		{
			...
			{				
				if(two_5_tap_enable)
				{
					_dispc_set_row_inc(plane, 1+width - ((width)%4));
				}
				else
				{
					_dispc_set_row_inc(plane, 1);
				}
			}
			else
			{
				if(two_5_tap_enable)
				{
					_dispc_set_row_inc(plane, row_inc + width - ((width)%4));
				}
				else
				{
					_dispc_set_row_inc(plane, row_inc);
		...
		else if(rotation==OMAP_DSS_ROT_180)
		{				
			if(two_5_tap_enable)
			{
				_dispc_set_row_inc(plane, row_inc+(width) - ((width)%4));
		...
		else	//((rotation==OMAP_DSS_ROT_90)||(rotation==OMAP_DSS_ROT_270))
		{
			if(two_5_tap_enable)
			{
					_dispc_set_row_inc(OMAP_DSS_VIDEO1, row_inc+(width*2) - ((width)%4)*2);
					_dispc_set_row_inc(OMAP_DSS_VIDEO2, row_inc+(width*2) - ((width)%4)*2);
			}
			else
			{
				_dispc_set_row_inc(plane, row_inc);
...
//&*&*&*JJ+++  //pos_x
	if (plane != OMAP_DSS_GFX) 
	{
		if(two_5_tap_enable)
		{
			_dispc_set_plane_pos(OMAP_DSS_VIDEO1, pos_x + (0), pos_y);
			_dispc_set_plane_pos(OMAP_DSS_VIDEO2, pos_x + (out_width/2), pos_y);
		}
		else
			_dispc_set_plane_pos(plane, pos_x, pos_y);
	}
...
//&*&*&*JJ+++  //pic_size
	if (plane != OMAP_DSS_GFX) 
	{
		if(two_5_tap_enable)
		{
			_dispc_set_pic_size(OMAP_DSS_VIDEO1, width/2 + (width/2)%2, height);
			_dispc_set_pic_size(OMAP_DSS_VIDEO2, width/2 + (width/2)%2, height);
		}
		else
			_dispc_set_pic_size(plane, width, height);
	}
...
//&*&*&*JJ+++  //vid_size
	...
	{
		if(two_5_tap_enable)
			_dispc_set_vid_size(plane, out_width/2, out_height);
		else
			_dispc_set_vid_size(plane, out_width, out_height);
	}
	else
...
//&*&*&*JJ1_20111102 EP10 panel for Video optimization
#ifdef CONFIG_EP10_PANEL_OPTIMIZATION
	REG_FLD_MOD(DISPC_GLOBAL_ALPHA, global_alpha, 23, 16);//&*&*&*JJ1
#endif
//&*&*&*JJ2_20111102 EP10 panel for Video optimization
...
//&*&*&*JJ1_20111102 EP10 panel for Video optimization
#ifdef CONFIG_EP10_PANEL_OPTIMIZATION
	if((plane == OMAP_DSS_VIDEO1) ||(plane == OMAP_DSS_VIDEO2))
	{ 
		REG_FLD_MOD(dispc_reg_att[OMAP_DSS_VIDEO1], 0, 0, 0);
		REG_FLD_MOD(dispc_reg_att[OMAP_DSS_VIDEO2], 0, 0, 0);
	}
...
{	
	...
	{
//		REG_FLD_MOD(dispc_reg_att[OMAP_DSS_VIDEO2], 0, 0, 0);
//	 dispc_write_reg(DISPC_VID_BA0							(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_BA0							(OMAP_DSS_VIDEO1 - 1)));
//	 dispc_write_reg(DISPC_VID_BA1							(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_BA1							(OMAP_DSS_VIDEO1 - 1)));
//	 dispc_write_reg(DISPC_VID_POSITION					(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_POSITION				(OMAP_DSS_VIDEO1 - 1)));
	 dispc_write_reg(DISPC_VID_SIZE							(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_SIZE						(OMAP_DSS_VIDEO1 - 1)));
	 dispc_write_reg(DISPC_VID_ATTRIBUTES				(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_ATTRIBUTES			(OMAP_DSS_VIDEO1 - 1)));
	 dispc_write_reg(DISPC_VID_FIFO_THRESHOLD		(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_FIFO_THRESHOLD	(OMAP_DSS_VIDEO1 - 1)));
	 dispc_write_reg(DISPC_VID_FIFO_SIZE_STATUS	(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_FIFO_SIZE_STATUS(OMAP_DSS_VIDEO1 - 1)));
	 dispc_write_reg(DISPC_VID_ROW_INC					(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_ROW_INC					(OMAP_DSS_VIDEO1 - 1)));
	 dispc_write_reg(DISPC_VID_PIXEL_INC				(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_PIXEL_INC				(OMAP_DSS_VIDEO1 - 1)));
	 dispc_write_reg(DISPC_VID_FIR							(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_FIR							(OMAP_DSS_VIDEO1 - 1)));
	 dispc_write_reg(DISPC_VID_PICTURE_SIZE			(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_PICTURE_SIZE		(OMAP_DSS_VIDEO1 - 1)));
	 dispc_write_reg(DISPC_VID_ACCU0						(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_ACCU0						(OMAP_DSS_VIDEO1 - 1)));
	 dispc_write_reg(DISPC_VID_ACCU1						(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_ACCU1						(OMAP_DSS_VIDEO1 - 1)));  
	 dispc_write_reg(DISPC_VID_PRELOAD					(OMAP_DSS_VIDEO2 - 1), dispc_read_reg(DISPC_VID_PRELOAD					(OMAP_DSS_VIDEO1 - 1)));  		
	 
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO2 - 1),0), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO1 - 1),0)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO2 - 1),1), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO1 - 1),1)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO2 - 1),2), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO1 - 1),2)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO2 - 1),3), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO1 - 1),3)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO2 - 1),4), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO1 - 1),4)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO2 - 1),5), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO1 - 1),5)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO2 - 1),6), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO1 - 1),6)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO2 - 1),7), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO1 - 1),7)));
	 
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO2 - 1),0), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO1 - 1),0)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO2 - 1),1), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO1 - 1),1)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO2 - 1),2), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO1 - 1),2)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO2 - 1),3), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO1 - 1),3)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO2 - 1),4), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO1 - 1),4)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO2 - 1),5), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO1 - 1),5)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO2 - 1),6), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO1 - 1),6)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO2 - 1),7), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO1 - 1),7)));
	 
	 dispc_write_reg(DISPC_VID_CONV_COEF				((OMAP_DSS_VIDEO2 - 1),0), dispc_read_reg(DISPC_VID_CONV_COEF		((OMAP_DSS_VIDEO1 - 1),0)));
	 dispc_write_reg(DISPC_VID_CONV_COEF				((OMAP_DSS_VIDEO2 - 1),1), dispc_read_reg(DISPC_VID_CONV_COEF		((OMAP_DSS_VIDEO1 - 1),1)));
	 dispc_write_reg(DISPC_VID_CONV_COEF				((OMAP_DSS_VIDEO2 - 1),2), dispc_read_reg(DISPC_VID_CONV_COEF		((OMAP_DSS_VIDEO1 - 1),2)));
	 dispc_write_reg(DISPC_VID_CONV_COEF				((OMAP_DSS_VIDEO2 - 1),3), dispc_read_reg(DISPC_VID_CONV_COEF		((OMAP_DSS_VIDEO1 - 1),3)));
	 dispc_write_reg(DISPC_VID_CONV_COEF				((OMAP_DSS_VIDEO2 - 1),4), dispc_read_reg(DISPC_VID_CONV_COEF		((OMAP_DSS_VIDEO1 - 1),4)));
	 
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO2 - 1),0), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO1 - 1),0)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO2 - 1),1), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO1 - 1),1)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO2 - 1),2), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO1 - 1),2)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO2 - 1),3), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO1 - 1),3)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO2 - 1),4), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO1 - 1),4)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO2 - 1),5), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO1 - 1),5)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO2 - 1),6), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO1 - 1),6)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO2 - 1),7), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO1 - 1),7)));	 
	 //REG_FLD_MOD(dispc_reg_att[OMAP_DSS_VIDEO2], 1, 0, 0);
	 REG_FLD_MOD(dispc_reg_att[OMAP_DSS_VIDEO2], enable ? 1 : 0, 0, 0);
	}
	else if (plane == OMAP_DSS_VIDEO2)
	{
//		REG_FLD_MOD(dispc_reg_att[OMAP_DSS_VIDEO1], 0, 0, 0);
//	 dispc_write_reg(DISPC_VID_BA0							(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_BA0							(OMAP_DSS_VIDEO2 - 1)));
//	 dispc_write_reg(DISPC_VID_BA1							(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_BA1							(OMAP_DSS_VIDEO2 - 1)));
//	 dispc_write_reg(DISPC_VID_POSITION					(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_POSITION				(OMAP_DSS_VIDEO2 - 1)));
	 dispc_write_reg(DISPC_VID_SIZE							(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_SIZE						(OMAP_DSS_VIDEO2 - 1)));
	 dispc_write_reg(DISPC_VID_ATTRIBUTES				(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_ATTRIBUTES			(OMAP_DSS_VIDEO2 - 1)));
	 dispc_write_reg(DISPC_VID_FIFO_THRESHOLD		(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_FIFO_THRESHOLD	(OMAP_DSS_VIDEO2 - 1)));
	 dispc_write_reg(DISPC_VID_FIFO_SIZE_STATUS	(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_FIFO_SIZE_STATUS(OMAP_DSS_VIDEO2 - 1)));
	 dispc_write_reg(DISPC_VID_ROW_INC					(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_ROW_INC					(OMAP_DSS_VIDEO2 - 1)));
	 dispc_write_reg(DISPC_VID_PIXEL_INC				(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_PIXEL_INC				(OMAP_DSS_VIDEO2 - 1)));
	 dispc_write_reg(DISPC_VID_FIR							(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_FIR							(OMAP_DSS_VIDEO2 - 1)));
	 dispc_write_reg(DISPC_VID_PICTURE_SIZE			(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_PICTURE_SIZE		(OMAP_DSS_VIDEO2 - 1)));
	 dispc_write_reg(DISPC_VID_ACCU0						(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_ACCU0						(OMAP_DSS_VIDEO2 - 1)));
	 dispc_write_reg(DISPC_VID_ACCU1						(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_ACCU1						(OMAP_DSS_VIDEO2 - 1)));  
	 dispc_write_reg(DISPC_VID_PRELOAD					(OMAP_DSS_VIDEO1 - 1), dispc_read_reg(DISPC_VID_PRELOAD					(OMAP_DSS_VIDEO2 - 1)));  		
	 
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO1 - 1),0), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO2 - 1),0)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO1 - 1),1), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO2 - 1),1)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO1 - 1),2), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO2 - 1),2)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO1 - 1),3), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO2 - 1),3)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO1 - 1),4), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO2 - 1),4)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO1 - 1),5), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO2 - 1),5)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO1 - 1),6), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO2 - 1),6)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_H				((OMAP_DSS_VIDEO1 - 1),7), dispc_read_reg(DISPC_VID_FIR_COEF_H	((OMAP_DSS_VIDEO2 - 1),7)));
	 
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO1 - 1),0), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO2 - 1),0)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO1 - 1),1), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO2 - 1),1)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO1 - 1),2), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO2 - 1),2)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO1 - 1),3), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO2 - 1),3)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO1 - 1),4), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO2 - 1),4)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO1 - 1),5), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO2 - 1),5)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO1 - 1),6), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO2 - 1),6)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_HV			((OMAP_DSS_VIDEO1 - 1),7), dispc_read_reg(DISPC_VID_FIR_COEF_HV	((OMAP_DSS_VIDEO2 - 1),7)));
	 
	 dispc_write_reg(DISPC_VID_CONV_COEF				((OMAP_DSS_VIDEO1 - 1),0), dispc_read_reg(DISPC_VID_CONV_COEF		((OMAP_DSS_VIDEO2 - 1),0)));
	 dispc_write_reg(DISPC_VID_CONV_COEF				((OMAP_DSS_VIDEO1 - 1),1), dispc_read_reg(DISPC_VID_CONV_COEF		((OMAP_DSS_VIDEO2 - 1),1)));
	 dispc_write_reg(DISPC_VID_CONV_COEF				((OMAP_DSS_VIDEO1 - 1),2), dispc_read_reg(DISPC_VID_CONV_COEF		((OMAP_DSS_VIDEO2 - 1),2)));
	 dispc_write_reg(DISPC_VID_CONV_COEF				((OMAP_DSS_VIDEO1 - 1),3), dispc_read_reg(DISPC_VID_CONV_COEF		((OMAP_DSS_VIDEO2 - 1),3)));
	 dispc_write_reg(DISPC_VID_CONV_COEF				((OMAP_DSS_VIDEO1 - 1),4), dispc_read_reg(DISPC_VID_CONV_COEF		((OMAP_DSS_VIDEO2 - 1),4)));
	 
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO1 - 1),0), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO2 - 1),0)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO1 - 1),1), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO2 - 1),1)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO1 - 1),2), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO2 - 1),2)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO1 - 1),3), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO2 - 1),3)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO1 - 1),4), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO2 - 1),4)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO1 - 1),5), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO2 - 1),5)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO1 - 1),6), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO2 - 1),6)));
	 dispc_write_reg(DISPC_VID_FIR_COEF_V				((OMAP_DSS_VIDEO1 - 1),7), dispc_read_reg(DISPC_VID_FIR_COEF_V	((OMAP_DSS_VIDEO2 - 1),7)));		
	 //REG_FLD_MOD(dispc_reg_att[OMAP_DSS_VIDEO1], 1, 0, 0);
	 REG_FLD_MOD(dispc_reg_att[OMAP_DSS_VIDEO1], enable ? 1 : 0, 0, 0);
	} 
	
	{
		//check video1&video2 postition overlay
		u32 vid1_pos,vid2_pos,vid1_size,vid2_size;
		vid1_pos = REG_GET(DISPC_VID_POSITION(OMAP_DSS_VIDEO1 - 1), 10, 0);
		vid2_pos = REG_GET(DISPC_VID_POSITION(OMAP_DSS_VIDEO2 - 1), 10, 0);
		vid1_size = REG_GET(DISPC_VID_SIZE(OMAP_DSS_VIDEO1 - 1), 10, 0);
		if((vid1_pos+vid1_size)>=(vid2_pos))
			printk("[JJErr]:(%s)vid1_pos=%d, vid2_pos=%d, vid1_size=%d, vid2_size=%d	\n",
			__FUNCTION__,vid1_pos,vid2_pos,vid1_size,vid2_size);
	}
}
...
#else
		REG_FLD_MOD(DISPC_CONFIG, 1, 18, 18);
#endif
//&*&*&*JJ2_20111102 EP10 panel for Video optimization
	else /* OMAP_DSS_CHANNEL_DIGIT */
...
void dispc_irq_handler(void)
{
	...
	memcpy(registered_isr, dispc.registered_isr, sizeof(registered_isr));
...
//&*&*&*JJ1_20110824 Bug #953:Touch is not workable after playback recorded video
		//omapdss_channel_reset(OMAP_DSS_CHANNEL_LCD);
//&*&*&*JJ2_20110824 Bug #953:Touch is not workable after playback recorded video
	}
...
int omap_dispc_wait_for_irq_timeout(u32 irqmask, unsigned long timeout)
{
	...
	r = omap_dispc_register_isr(dispc_irq_wait_handler, &completion, irqmask);
	if (r)
		return r;
