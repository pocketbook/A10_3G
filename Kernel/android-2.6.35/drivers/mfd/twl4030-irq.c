...
static irqreturn_t dummy_twl4030_irq(int irq, void *devid)
{
	//printk("dummy_twl4030_irq ..., irq=%d\n", irq);
...
//&*&*&*HC1_20110706, modify to prevent unhandled sys_nirq trigger
	...
	status = request_irq(OMAP_GPIO_IRQ(0), dummy_twl4030_irq, IRQF_TRIGGER_LOW|IRQF_NO_SUSPEND, "dummy_twl4030_irq", &irq_event);

	if (status)			
		pr_err("twl4030: could not dummy irq%d: %d\n", OMAP_GPIO_IRQ(0), status);

	disable_irq(OMAP_GPIO_IRQ(0));
//&*&*&*HC2_20110706, modify to prevent unhandled sys_nirq trigger
