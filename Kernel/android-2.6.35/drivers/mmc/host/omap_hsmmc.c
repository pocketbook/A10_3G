...
#if defined (CONFIG_SD_CARD_WAKEUP) || defined (CONFIG_SIM_CARD_WAKEUP)
	struct	delayed_work	mmc_carddetect_work;
#else
...
//&*&*&*HC1_20110718, Add SD In/Out wakeup
#if defined (CONFIG_SD_CARD_WAKEUP) 
	// do not enable/disable irq, if we enable sd wakeup function
#else
...
int g_sim_carddetect_status_new = 0;
...
//&*&*&*HC1_20110718, Add SD In/Out wakeup
#if defined (CONFIG_SD_CARD_WAKEUP) 
	// Bring the screen to on if device goes through earlysuspend state
	if((host->enter_early_suspend == 1) && (TWL4030_GPIO0_IRQ == slot->card_detect_irq))
	{
		if (host->wakeup_event == 0)
		{
			SendPowerbuttonEvent();
			printk("Enter %s, send power suspend event.\n", __FUNCTION__);
			host->wakeup_event = 1;
		}	

		// delay the work to prevent damaging the earlysuspend work
		schedule_delayed_work(&host->mmc_carddetect_work, 2*HZ);
		return;
	}
#endif	
//&*&*&*HC2_20110718, Add SD In/Out wakeup
...
//&*&*&*SJ1_20110607, Add SIM card detection.	
#if defined (CONFIG_SIM_CARD_DETECTION) && defined (CONFIG_CHANGE_INAND_MMC_SCAN_INDEX)	
	if (TWL4030_GPIO1_IRQ == slot->card_detect_irq ) {
		//		printk("carddetect=%d\n", carddetect);
		if (g_sim_carddetect_status == carddetect) {
			msleep(200);			
			return;
		}
//&*&*&*SJ1_20110824, To avoid EHCI load/unload module crash issue when the SIM card insert/remove.
/*
		g_sim_carddetect_status = carddetect;
		printk("g_sim_carddetect_status=%d\n", g_sim_carddetect_status);
		if (carddetect)
			sim_card_detect_change(host->mmc, (HZ * 200) / 1000);
		else
			sim_card_detect_change(host->mmc, (HZ * 200) / 1000);
*/
		g_sim_carddetect_status_new = carddetect;
		sim_card_detect_change(host->mmc, (HZ * 500) / 1000);
//&*&*&*SJ2_20110824, To avoid EHCI load/unload module crash issue when the SIM card insert/remove.
	} else {
		if (carddetect) {
			g_mmc1_poll_cnt = 0;	 //&*&*&*SJ_20110721.
			mmc_detect_change(host->mmc, (HZ * 200) / 1000);
		} else {
			g_mmc1_poll_cnt = 1;   //&*&*&*SJ_20110721.
			mmc_detect_change(host->mmc, (HZ * 200/*50*/) / 1000); //&*&*&*SJ_20110721, increase delay time.
		}
	}
#else
...
//&*&*&*HC1_20111206, Modify for SIM card wakeup
#if defined (CONFIG_SIM_CARD_WAKEUP) 
	if (host->suspended && (irq==TWL4030_GPIO1_IRQ)) {
		// if interrupt occurs during suspend, schedule a delay work to handle it
		schedule_delayed_work(&host->mmc_carddetect_work, 100);
		return IRQ_HANDLED;
	}	
	...
	if (host->suspended)
		return IRQ_HANDLED;
#endif
//&*&*&*HC2_20111206, Modify for SIM card wakeup

//&*&*&*HC1_20110803, Modify for early suspend blocking issue
#if defined (CONFIG_SD_CARD_WAKEUP) || defined (CONFIG_SIM_CARD_WAKEUP)
	schedule_delayed_work(&host->mmc_carddetect_work, 0);
#else
...
#if defined (CONFIG_SIM_CARD_DETECTION) && defined (CONFIG_CHANGE_INAND_MMC_SCAN_INDEX)
//	printk("pdev->id=%d\n", pdev->id);
	if (OMAP_MMC2_DEVID == pdev->id) {
	ret = omap_hsmmc_gpio_sim_init(pdata);
	if (ret)
		goto err;
	} else {
...
//&*&*&*HC1_20110718, Add SD In/Out wakeup
	...
	host->wakeup_event = 0;
#ifdef CONFIG_HAS_EARLYSUSPEND
	host->early_suspend.suspend = omap_hsmmc_early_suspend;
	host->early_suspend.resume = omap_hsmmc_late_resume;
...
//&*&*&*HC1_20110803, Modify for early suspend blocking issue
#if defined (CONFIG_SD_CARD_WAKEUP) || defined (CONFIG_SIM_CARD_WAKEUP)
	INIT_DELAYED_WORK(&host->mmc_carddetect_work, omap_hsmmc_detect);
#else
...
//&*&*&*HC2_20110803, Modify for early suspend blocking issue	
	...
	mmc->f_min	= 400000;
	mmc->f_max	= 52000000;
	// mmc->f_min	= 140000; //400000;
	// mmc->f_max	= 8000000; //52000000;
	...
	//&*&*&*JohWan1 Modify for wifi driver DHD ver.218.248 data throughput improvement
	if(host->id != OMAP_MMC3_DEVID )
		mmc_set_disable_delay(mmc, OMAP_MMC_DISABLED_TIMEOUT);
	else
		mmc_set_disable_delay(mmc, 10); //mmc3 wakelock delay set 10m sec

	//&*&*&*JohWan2 Modify for wifi driver DHD ver.218.248 data throughput improvement
	...
	printk("before register in mmc%d\n", host->id);

	/* Request IRQ for card detect */
	// if ((mmc_slot(host).card_detect_irq)) {
	if ( (mmc_slot(host).card_detect_irq) && (host->id !=2) ) {		
		printk("mmc%d, slot_id = %d\n", host->id);
...
#if defined (CONFIG_SIM_CARD_DETECTION) && defined (CONFIG_CHANGE_INAND_MMC_SCAN_INDEX)
		if (OMAP_MMC2_DEVID == pdev->id) {
			if (0 == sim_detect_alloc()) {
				the_sim_detect->sdev.name = "sim_card_socket";			
				the_sim_detect->sdev.print_name = print_sim_card_switch_name;			
				the_sim_detect->sdev.print_state = print_sim_card_switch_state;			
				INIT_DELAYED_WORK(&the_sim_detect->sim_detect_delayed_work, update_sim_card_status);	
				if (switch_dev_register(&the_sim_detect->sdev) < 0)
					printk("%s switch_dev_register error\n", the_sim_detect->sdev.name);
				else
					the_sim_detect->sdev.state = ~0;
			}
		}
#endif
//&*&*&*SJ2_20110607, Add SIM card detection.
