...
//&*&*&*SJ1_20110824, To avoid EHCI load/unload module crash issue when the SIM card insert/remove.
{
	...
	for (i=0 ; i < 5 ; i++) {
		value = gpio_get_value_cansleep(TWL4030_GPIO1);
		if (g_sim_carddetect_status_new != value)
			return;

		msleep(200);
	}
	g_sim_carddetect_status = g_sim_carddetect_status_new;
...
#endif
//&*&*&*HC220111206, Modify for SIM card wakeup
}
