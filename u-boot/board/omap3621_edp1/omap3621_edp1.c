...
enum BatteryID_enum g_BatteryID; //&*&*&*SJ_20110704
...
int gpio_init (void)
{
	...
	/** Set KEY_LIGHT_EN GPIO_91 as output low **/
    sr32((u32)&gpio3_base->oe, 27, 1, 0);
	sr32((u32)&gpio3_base->setdataout, 27, 1, 0);

	/** Set  OTG_EN_1V8 GPIO_156 as output low **/
	sr32((u32)&gpio5_base->oe, 28, 1, 0);
	sr32((u32)&gpio5_base->res2[0], 28, 1, 0); 

	/** Set CHG_OFFMODE GPIO_34 as output low **/
	sr32((u32)&gpio2_base->oe, 2, 1, 0);
	sr32((u32)&gpio2_base->res2[0], 2, 1, 0); 	
...
enum BoardID_enum GetHardwareID(unsigned int val)
{
	...
	else if ((val > 0)&&(val <= 345))     id = BOARD_ID_EVT3; /*id = BOARD_ID_PRO;*/
	else if ((val > 345) &&(val <= 614))	id = BOARD_ID_EVT1;
	else if ((val > 614) &&(val <= 835))	id = BOARD_ID_EVT2;
	else if ((val > 835) &&(val <= 1010))	id = BOARD_ID_DVT1;
	else if ((val > 1010)&&(val <= 1223))	id = BOARD_ID_DVT2;
	else if ((val > 1223)&&(val <= 1413))	id = BOARD_ID_PVT;
	else if (val > 1413)                    id = BOARD_ID_MP;
...
//&*&*&*SJ1_20110704, add battery barcode info.
char g_szBatteryBarCode[512] = {0,};
enum BatteryID_enum GetBatteryID(unsigned int val)
{
	char cmd[48];
	enum BatteryID_enum id;
	
	mmc_init(1);
	sprintf(cmd, "mmc 1 read %p 2dffe00 200", g_szBatteryBarCode);
	run_command(cmd, 0);
	if ((g_szBatteryBarCode[0] == 'E') && (g_szBatteryBarCode[1] == 'A')) {
		if (g_szBatteryBarCode[2] == 'L') {
			id = BATTERY_ID_LICO;
			printf("BATT:  Lico  [%s]\n", g_szBatteryBarCode);
		} else if (g_szBatteryBarCode[2] == 'M') {
			id = BATTERY_ID_MCNAIR;
			printf("BATT:  Mcnair[%s]\n", g_szBatteryBarCode);
		}
	} else {
		id = BATTERY_ID_UNKNOWN;
		memset(g_szBatteryBarCode, 0x0, 16);
		puts("BATT:  UNKNOWN\n");
	}
	
	return id;
}
//&*&*&*SJ2_20110704, add battery barcode info.
...
//&*&*&*SJ2_20110607, Add check 3G modem exist GPIO status for EVT3. 

//&*&*&*SJ1_20110607, Add check recovery flag. 
...
//&*&*&*SJ2_20110607, Add check recovery flag. 
...
#define MUX_DEFAULT_ES2()\
	...
	/*GPMC*/\
	MUX_VAL(CP(GPMC_A1),        (IDIS  | PTD | EN | OFF_IN_PD | M4)) /*GPIO-34, CHG_OFFMODE(EVT2)*/\	
	MUX_VAL(CP(GPMC_A2),        (IDIS | PTD | DIS | M4)) /*GPIO-35, 3G SYSTEM_RESET_N(EVT2) */\
	MUX_VAL(CP(GPMC_A3),        (IDIS | PTD | EN |  M4)) /*GPIO-36 MODEM_POW_EN(EVT2), enable pulled down to avoid inrush current*/\
	MUX_VAL(CP(GPMC_A4),        (IDIS | PTD | DIS | OFF_OUT_PD | M4)) /*GPIO-37, LVDS SHTDN_N*/\
	MUX_VAL(CP(GPMC_A5),        (IDIS | PTD | DIS  | OFF_OUT_PD | M4)) /*GPIO-38, LCD_BL_EN */\
	...
	MUX_VAL(CP(GPMC_A8),        (IEN  | PTU | EN  /*| OFF_IN_PD*/ | M4)) /*GPIO-41 IOC_USB1_ID, keep same state to avoid unexpected wakeup trigger*/\
	MUX_VAL(CP(GPMC_A9),        (IDIS  | PTU | DIS | M4)) /*GPMC_A9, GPIO-42, MAX8677_CEN */\
	MUX_VAL(CP(GPMC_A10),       (IEN  | PTD | DIS | OFF_IN_PD | M4)) /*GPIO-43, CHG_FAULT(EVT2) */\
	...
	MUX_VAL(CP(GPMC_D8),        (IEN  | PTU |  EN  | OFF_IN_PU | M4)) /*GPIO-44, VOLT_INT(EVT2), remain same state in sleep*/\
	MUX_VAL(CP(GPMC_D9),        (IDIS | PTU | EN | OFF_IN_PU | M4)) /*GPIO-45, CCM_PWDN*/\
	MUX_VAL(CP(GPMC_D10),       (IDIS | PTD | DIS | M4)) /*GPIO-46, CCM_RESET_N*/\
	MUX_VAL(CP(GPMC_D11),       (IDIS | PTD | EN | OFF_IN_PD | M4)) /*GPIO-47, LCD_PWR_EN(EVT2) */\
	MUX_VAL(CP(GPMC_D12),       (IEN | PTD | DIS | OFF_IN_PD | M4)) /*GPIO-48, IOC I2C_REQ*/\
	MUX_VAL(CP(GPMC_D13),       (IDIS | PTU | EN | OFF_IN_PD | M4)) /*GPIO-49, iNAND_POW */\
	MUX_VAL(CP(GPMC_D14),       (IEN | PTU | EN | OFF_IN_PU| M4)) /*GPIO-50, CCM_2V8_EN(EVT2) */\
	...
	MUX_VAL(CP(GPMC_nBE0_CLE),  (IEN | PTU | DIS | OFF_IN_PD | M4)) /*GPI0-60, up_IOC_LEVEL(EVT2)*/\
	MUX_VAL(CP(GPMC_nBE1),      (IDIS | PTD | DIS | M4)) /*GPI0-61, USB_MODE*/\	
	MUX_VAL(CP(GPMC_nWP),       (IEN | PTD | EN | OFF_IN_PD | M4)) /*GPI0-62, GPS_WAKEUP(EVT2)*/\
	...
	MUX_VAL(CP(DSS_DATA17),     (IDIS | PTU | DIS | OFF_IN_PU | M0)) /*DSS_DATA17, GPIO-87*/\
	MUX_VAL(CP(DSS_DATA18),     (IDIS | PTD | EN 	| OFF_IN_PD | M4)) /*GPIO-88, LCD_PWR_EN(EVT3) */\
	MUX_VAL(CP(DSS_DATA19),     (IEN  | PTD | EN  | OFF_IN_PU /*| OFF_IN_PD*/ | M4)) /*DSS_DAT19 BCM4329 WL_IRQ*/\
	MUX_VAL(CP(DSS_DATA20),     (IEN  | PTD | EN  | OFF_IN_PD | M4)) /*DSS_DATA20, GPIO-90, LVDS_CLKSEL(EVT2)*/\
	MUX_VAL(CP(DSS_DATA21),     (IDIS | PTD | DIS | OFF_IN_PD | M4)) /*DSS_DATA21*/\
	MUX_VAL(CP(DSS_DATA22),     (IDIS | PTD | DIS | OFF_IN_PD | M7)) /*DSS_DATA22*/\
	MUX_VAL(CP(DSS_DATA23),     (IDIS | PTD | DIS | OFF_IN_PD | M7)) /*DSS_DATA23*/\
	/*McSPI1 */\
	...
	MUX_VAL(CP(CAM_XCLKA),      (IEN  | PTD | DIS | OFF_IN_PD | M4)) /*GPIO-96, GPS_WAKEUP*/\
	...
	MUX_VAL(CP(CAM_D0 ),        (IEN  | PTU | DIS | M4)) /*GPIO-99, HPH_DETECT(EVT2)| OFF_IN_PD */\
	MUX_VAL(CP(CAM_D1 ),        (IEN  | PTD | EN  | OFF_IN_PD | M4)) /*GPIO-100*/\
	MUX_VAL(CP(CAM_D2 ),        (IDIS | PTD | DIS | OFF_IN_PD | M4)) /*GPIO-101, GPS_RESET*/\
	MUX_VAL(CP(CAM_D3 ),        (IDIS | PTD | DIS | M4)) /*BT_RST_N, GPIO-102*/\
	MUX_VAL(CP(CAM_D4 ),        (IEN  | PTD | DIS | M4)) /*GPIO-103*/\
	...
	MUX_VAL(CP(CAM_D11),        (IDIS | PTD | DIS | OFF_IN_PD | M4)) /*GPIO-110, UART1_SW*/\
	...
	MUX_VAL(CP(UART1_RTS),      (IEN  | PTD | DIS | OFF_IN_PD | M4)) /*UART1_RTS, GPIO-149, CHG*/\
	MUX_VAL(CP(UART1_CTS),      (IEN  | PTU | EN  | OFF_IN_PD | M4)) /*UART1_CTS, GPIO_150, IOC_RST*/\
	MUX_VAL(CP(UART1_RX),       (IEN  | PTD | DIS | OFF_IN_PD | M0)) /*UART1_RX*/\
	MUX_VAL(CP(McBSP1_CLKR),    (IDIS  | PTD | EN | OFF_IN_PD | M4)) /*GPIO-156, OTG_EN_1V8(EVT2)*/\
	MUX_VAL(CP(McBSP1_FSR),     (IEN  | PTD | DIS | OFF_IN_PD | M7)) /*GPIO_157*/\
	MUX_VAL(CP(McBSP1_DX),      (IDIS | PTD | DIS | OFF_IN_PD | M4)) /*GIO-158, WL_RST_N*/\
	MUX_VAL(CP(McBSP1_DR),      (IDIS | PTD | DIS | OFF_IN_PD | M4)) /*GIO-159, GPS_POWON*/\
	MUX_VAL(CP(McBSP_CLKS),     (IEN  | PTD | EN  | OFF_IN_PD | M4)) /*GIO-160*/\
	MUX_VAL(CP(McBSP1_FSX),     (IEN  | PTD | EN  | OFF_IN_PD | M4)) /*GIO-161*/\
	/*McBSP4 Interface */\
	...
	MUX_VAL(CP(JTAG_EMU0),      (IEN  | PTD | DIS | OFF_IN_PU/*OFF_IN_PD*/ | M4)) /*GIPO-11, UP_IOC_DATA_GPIO(EVT2), Herry modify*/\
	MUX_VAL(CP(JTAG_EMU1),      (IEN  | PTU | DIS | OFF_IN_PU/*OFF_IN_PD*/ | M4 )) /*GPIO-31, UP_IOC_CLK_GPIO, Herry modify*/\
	MUX_VAL(CP(ETK_CTL_ES2),    (IEN  | PTD | DIS | OFF_IN_PU/*| OFF_IN_PD*/ | M2)) /*MMC3_CMD, Herry modify */\
	MUX_VAL(CP(ETK_CLK_ES2),    (IEN  | PTU | EN  | OFF_IN_PU/*| OFF_IN_PD*/ | M2)) /*MMC3_CLK, Herry modify  */\
	MUX_VAL(CP(ETK_D0_ES2 ),    (IEN  | PTU | EN  | OFF_IN_PD | M4)) /*GIO-14, TOUCH_INT(EVT2)  */\
	MUX_VAL(CP(ETK_D1_ES2 ),    (IDIS | PTU | EN | M4)) /*GIO-15, EN_AUDIO_POW   */\
	MUX_VAL(CP(ETK_D2_ES2 ),    (IEN | PTD | EN | OFF_IN_PD | M4)) /*GIPO-16 IOC_POWER */\
	MUX_VAL(CP(ETK_D4_ES2 ),    (IEN  | PTU | EN  | OFF_IN_PU/*| OFF_IN_PD*/ | M2)) /*MMC3_DAT0, Herry modify */\
	MUX_VAL(CP(ETK_D5_ES2 ),    (IEN  | PTU | EN  | OFF_IN_PU/*| OFF_IN_PD*/ | M2)) /*MMC3_DAT1, Herry modify */\
	MUX_VAL(CP(ETK_D6_ES2 ),    (IEN  | PTU | EN  | OFF_IN_PU/*| OFF_IN_PD*/ | M2)) /*MMC3_DAT2, Herry modify */\
	MUX_VAL(CP(ETK_D3_ES2 ),    (IEN  | PTU | EN  | OFF_IN_PU/*| OFF_IN_PD*/ | M2)) /*MMC3_DAT3, Herry modify */\
	MUX_VAL(CP(ETK_D7_ES2 ),    (IDIS | PTD | DIS | M4)) /*GIO-21, W_DISABLE_N(EVT2) */\
	MUX_VAL(CP(ETK_D8_ES2 ),    (IDIS | PTU | EN | OFF_OUT_PU/*| OFF_OUT_PD*/ | M4)) /*WL_BT_POW_EN, GIO-22 Always ON*/\
	MUX_VAL(CP(ETK_D9_ES2 ),    (IDIS | PTD | DIS | M4)) /*GIO-23  Phy-Reset  */\
...
/*	MUX_VAL(CP(DSS_DATA18),     (IDIS | PTD | DIS | OFF_IN_PD | M4))*/ /*DSS_DATA18*/\
	MUX_VAL(CP(DSS_DATA19),     (IEN  | PTD | EN  | OFF_IN_PU /*| OFF_IN_PD*/ | M4/*M0*/)) /*DSS_DATA19 BCM4329 WL_IRQ*/\
	MUX_VAL(CP(DSS_DATA20),     (IDIS | PTD | DIS | OFF_IN_PD | M4/*M0*/)) /*DSS_DATA20*/\
	MUX_VAL(CP(DSS_DATA21),     (IDIS | PTD | DIS | OFF_IN_PD | M4/*M0*/)) /*DSS_DATA21*/\
	MUX_VAL(CP(DSS_DATA22),     (IDIS | PTD | DIS | OFF_IN_PD | M4/*M0*/)) /*DSS_DATA22*/\
	MUX_VAL(CP(DSS_DATA23),     (IEN  | PTU | EN  | OFF_IN_PD | M4/*M0*/)) /*DSS_DATA23, GPIO-93, 3G_EXIST(EVT3)*/\
	MUX_VAL(CP(I2C2_SCL),       (IEN  | PTU | EN  | M0)) /*I2C2_SCL  */\
	MUX_VAL(CP(I2C2_SDA),       (IEN  | PTU | EN  | M0)) /*I2C2_SDA  */\
...
/*	MUX_VAL(CP(GPMC_nCS4),      (IDIS | PTD | DIS | OFF_IN_PD | M4))*/ /*GIO-55 */\
	/*MUX_VAL(CP(CAM_XCLKA),      (IEN  | PTD | DIS | OFF_IN_PD | M4))*/ /*GIO-96*/\
...
//&*&*&*SJ1_20110718, check second loader.
#if defined(CONFIG_CHECK_SECOND_BOOTLOADER)
extern int mmc_flag[];
void check_second_bootloader(void)
{
	int result;
	char *mmc_init[2] = {"mmcinit", "0",};
	
	if (do_mmc(NULL, 0, 2, mmc_init)) {
		return;	
	}

	run_command("fatload mmc 0:1 80f80000 cdpg-second-bootloader.ep10", 0);
	...
	if (result) {
		...
		puts ("*****************************************************\n");
		puts ("** [ <eMMC> U-Boot ] => [ <SD> Second BootLoader ] **\n");
		puts ("*****************************************************\n");
		run_command ("go 80f80000", 0);
	}
}
#endif
//&*&*&*SJ2_20110718, check second loader.
