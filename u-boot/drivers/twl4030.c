...
int twl4030_bootup_reason_get(void)
{
	int ret = 0;
	u8 value = 0;	
	u8 ret_val = 0;	

	ret = twl4030_i2c_read_u8(TWL4030_CHIP_PIH, &value, REG_PIH_ISR_P1);

///	printf(" PIH_ISR_P1=0x%x\n", value);
	
	if (ret) {
		printf(" read PIH_ISR_P1 fail, ret=%d\n", value);
		return ret;
	}

	if (value & 0x20)
	{
		ret = twl4030_i2c_read_u8(TWL4030_CHIP_INT, &value, REG_PWR_ISR1);

///		printf(" PWR_ISR1=0x%x\n", value);

		if (ret) {
			printf(" read PWR_ISR1 fail, ret=%d\n", value);
			return ret;
		}
		else{
			...
			while (value)
			{
				twl4030_i2c_write_u8(TWL4030_CHIP_INT, value, REG_PWR_ISR1);	

				twl4030_i2c_read_u8(TWL4030_CHIP_INT, &value, REG_PWR_ISR1);
///				printf("clear PWR_ISR1=0x%x\n",value);
			}
...
void twl4030_alarm_it_enable (u8 enable)
{
	u8 rtc_val = 0;	
	u8 value = 0;	

	// clear ALARM bit to disable trigger
	twl4030_i2c_read_u8(TWL4030_CHIP_RTC, &value, RTC_STATUS_REG);
	printf(" RTC_STATUS_REG=0x%x\n", value);	
	twl4030_i2c_write_u8(TWL4030_CHIP_RTC, value, RTC_STATUS_REG);	

	
	twl4030_i2c_read_u8(TWL4030_CHIP_RTC, &rtc_val, RTC_INTERRUPTS_REG);
	printf(" read RTC_INTERRUPTS_REG = 0x%x\n", rtc_val);
	...
	twl4030_i2c_write_u8(TWL4030_CHIP_RTC, rtc_val, RTC_INTERRUPTS_REG);
...
void twl4030_poweroff(void)
{
	...
	printf("Enter twl4030_poweroff...\n");
	
	/* Make sure SEQ_OFFSYNC is set so that all the res goes to wait-on */
	err = twl4030_i2c_read_u8(TWL4030_CHIP_PM_MASTER, &val,
				   PM_MASTER_CFG_P123_TRANSITION);
	if (err) {
		printf("I2C error %d while reading TWL4030 PM_MASTER CFG_P123_TRANSITION\n", err);
		return;
	}

	val |= SEQ_OFFSYNC;
	err = twl4030_i2c_write_u8(TWL4030_CHIP_PM_MASTER, val,
				    PM_MASTER_CFG_P123_TRANSITION);
	if (err) {
		printf("I2C error %d while writing TWL4030 PM_MASTER CFG_P123_TRANSITION\n", err);
		return;
	}
	...
	if (err) {
		printf("I2C error %d while reading TWL4030 PM_MASTER P1_SW_EVENTS\n", err);
		return;
	}
	...
	if (err) {
		printf("I2C error %d while writing TWL4030 PM_MASTER P1_SW_EVENTS\n", err);
		return;
	}
}
...
int twl4030_hw_sts_get(void)
{
	...
	//printf("Enter twl4030_hw_sts_get...\n");
	
	/* Make sure SEQ_OFFSYNC is set so that all the res goes to wait-on */
	err = twl4030_i2c_read_u8(TWL4030_CHIP_PM_MASTER, &val,
				   PM_MASTER_STS_HW_CONDITIONS);
	if (err) {
		printf("I2C error %d while reading PM_MASTER_STS_HW_CONDITIONS\n", err);
		return err;
	}

	//printf("Exit twl4030_hw_sts_get, val=0x%x\n", val);
	return val;
}
...
//&*&*&*SJ1_20101001, Add Get HardwareID.
	/* setting MDC channel 0 to trigger by SW1 */
	ret = clear_n_set(TWL4030_CHIP_MADC, 0, SW1_CH0_SEL, 	REG_SW1SELECT_LSB);
	if (ret) {
		puts("Setting MDC channel 0 error.\n");
		return ret;
	}	
//&*&*&*SJ1_20101001, Add Get HardwareID.
//&*&*&*AL1_20110304, Add VBAT Valtage detection
	/* setting MDC channel 9 and 12 to trigger by SW1 */
	ret = clear_n_set(TWL4030_CHIP_MADC, 0, SW1_CH12_SEL, REG_SW1SELECT_MSB);
	if (ret) {
		puts("Setting MDC channel 9 and channel 12 error.\n");
		return ret;
	}	
//&*&*&*AL2_20110304, Add VBAT Valtage detection
//&*&*&*AL1_20110304, Enable prescaler for channel12's VBAT converter 
	ret = clear_n_set(TWL4030_CHIP_MAIN_CHARGE, 0, REG_TWL4030_BCI_MESBAT_EN, REG_BCIA_CTRL);
	if (ret) {
		puts("Setting MDC prescale of channel 12 error.\n");
		return ret;
	}
//&*&*&*AL2_20110304, Enable prescaler for channel12's VBAT converter 
...
int twl4030_get_battery_voltage(void)
{
	...
	ret = twl4030_i2c_read_u8(TWL4030_CHIP_MADC, &volt_msb, GPCH12_MSB);
	if (ret)
		return ret;

	temp = ((int) volt_msb) << 2;

	ret = twl4030_i2c_read_u8(TWL4030_CHIP_MADC, &volt_lsb, GPCH12_LSB);
	if (ret)
		return ret;

	temp = temp + ((int) ((volt_lsb & MADC_LSB_MASK) >> 6));
 ...
 */
int twl4030_get_backup_battery_voltage(void)
{
	int ret, temp;
	u8 volt;
...
//&*&*&*SJ1_20101004, Add Get HardwareID.
/*
 * Return the ADCIN0 voltage
 * ADCIN0: General-purpose input
 * ANALOG INPUT RANGE [V] Max=1.5V Min=0V 
* Divider Ratio 1
 */
int twl4030_get_ADCIN0_voltage(void)
{
	int ret, temp;
	u8 volt;
	...
	ret = twl4030_i2c_read_u8(TWL4030_CHIP_MADC, &volt, GPCH0_MSB);
	if (ret)
		return ret;

	temp = ((int) volt) << 2;

	ret = twl4030_i2c_read_u8(TWL4030_CHIP_MADC, &volt, GPCH0_LSB);
	if (ret)
		return ret;

	temp = temp + ((int) ((volt & MADC_LSB_MASK) >> 6));

	return  (temp*1466)/1000;
}

int twl4030_madc_init(void)
{
	int ret;
...
int twl4030_vadc_onoff(int onoff)
{
	if (onoff) {
		/* turn on VDAC regulator */
		twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x20, 0x96);
		twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, 0x97);	
		twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x02, 0x98);
		udelay(50000);
	} else {
		/* turn off VDAC regulator */
		twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, 0x96);
		twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, 0x97);	
		twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x02, 0x98);
	}
	return 0;
}
//&*&*&*SJ2_20101004, Add Get HardwareID.
...
void twl4030_usb_ldo_off(void)
{
	printf("twl4030_usb_ldo_off +\n");

	/* input to VUSB3V1 LDO is from VBAT, not VBUS */
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x14, PM_RECEIVER_VUSB_DEDICATED1);

	/* turn off 3.1V regulator */
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, PM_RECEIVER_VUSB3V1_DEV_GRP);
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, PM_RECEIVER_VUSB3V1_TYPE);
	
	/* put VUSB3V1 LDO in active state */
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, PM_RECEIVER_VUSB_DEDICATED2);

	/* turn off 1.5V regulator */
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, PM_RECEIVER_VUSB1V5_DEV_GRP);
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, PM_RECEIVER_VUSB1V5_TYPE);

	/* turn off 1.8V regulator */
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, PM_RECEIVER_VUSB1V8_DEV_GRP);
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, PM_RECEIVER_VUSB1V8_TYPE);
...
//&*&*&*HC1_20110304, Add offmode charging
int twl4030_get_cable_type(void)
{
	...
	err = twl4030_i2c_read_u8(TWL4030_CHIP_PM_MASTER, &val,
				  PM_MASTER_STS_HW_CONDITIONS);
	if (err) {
		printf("I2C error %d while reading TWL4030 PM_MASTER PM_MASTER_STS_HW_CONDITIONS\n", err);
		return -1;
	}
	otg_ctrl = twl4030_usb_read(USB_OTG_CTRL);
///	printf("[twl4030_enable_charger_detection]USB_OTG_CTRL is 0x%2x\n", otg_ctrl);
	if (val & ((1<<7) | (1<<2))) {
		if (val & (1<<2))
			status = USB_EVENT_ID;
		else {
			/* Read charger detection status */
			err = twl4030_i2c_read_u8(TWL4030_CHIP_MAIN_CHARGE, &dtct, REG_USB_DTCT_CTRL);
			if (err)
			{
				printf("I2C error %d while reading TWL4030 TWL4030_CHIP_MAIN_CHARGE REG_USB_DTCT_CTRL\n", err);
				return err;
			}
...
///		printf("[twl4030-usb]USB_DTCT_CTRL=0x%02x, STS_HW_CONDITIONS=0x%02x, status=0x%02x\n", dtct, val, status);
	}
	return status;
}

static void twl4030_enable_charger_detection(void)
{
	int temp;
	twl4030_i2c_write_u8(TWL4030_CHIP_MAIN_CHARGE, 0x02, REG_USB_DTCT_CTRL);
	twl4030_i2c_write_u8(TWL4030_CHIP_MAIN_CHARGE, 0x01, REG_USB_DTCT_CTRL);
	//twl4030_i2c_write_u8(TWL4030_CHIP_MAIN_CHARGE, 0xb0, USB_SW_CHRG_CTRL);

	temp = twl4030_usb_read(USB_OTG_CTRL);
	printf("[twl4030_enable_charger_detection]USB_OTG_CTRL is 0x%2x\n", temp);
	//temp = temp & ~(7 << 0);
	//twl4030_usb_write(USB_OTG_CTRL, temp);
	
	//printf("[twl4030_enable_charger_detection]charger detection H/W FSM \n");
}
//&*&*&*HC2_20110304, Add offmode charging
...
//&*&*&*HC1_20110304, Add offmode charging
	twl4030_enable_charger_detection();
	mdelay(50);
//&*&*&*HC2_20110304, Add offmode charging	
...
//&*&*&*HC1_20110304, Add offmode charging
	/* Enable ULPI mode */
	printf("[twl4030_usb_init]USB_FUNC_CTRL=0x%02x. - enter\n", twl4030_usb_read(USB_FUNC_CTRL));
//&*&*&*BC1_110907:	usb can not reconnect after factory reset
//	twl4030_usb_write(USB_FUNC_CTRL, FUNC_CTRL_SUSPENDM | FUNC_CTRL_OPMODE_NONDRIVING);
...
//	twl4030_usb_write(USB_OTG_CTRL, TWL4030_OTG_CTRL_DMPULLDOWN | TWL4030_OTG_CTRL_DPPULLDOWN);
//&*&*&*BC2_110907:	usb can not reconnect after factory reset
	printf("[twl4030_usb_init]USB_FUNC_CTRL=0x%02x. - leave\n", twl4030_usb_read(USB_FUNC_CTRL));
//&*&*&*HC2_20110304, Add offmode charging	
...
//&*&*&*HC1_20110525, add for AC cable detection if booting from iNAND
	/* switch VBU1V5 and VBS1V8 to off then on for triggering the cable detection */
	...
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x20, PM_RECEIVER_VUSB1V8_DEV_GRP);
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_RECEIVER, 0x00, PM_RECEIVER_VUSB1V8_TYPE);
//&*&*&*HC2_20110525, add for AC cable detection if booting from iNAND

//&*&*&*HC1_20111025, disable STARTON_USB to prevent booting from unexpected source
	// disable protect key 
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_MASTER, R_KEY_1, PM_MASTER_PROTECT_KEY);
	twl4030_i2c_write_u8(TWL4030_CHIP_PM_MASTER, R_KEY_2, PM_MASTER_PROTECT_KEY);

	twl4030_i2c_read_u8(TWL4030_CHIP_PM_MASTER, &value, PM_MASTER_CFG_P1_TRANSITION);

	if (value & STARTON_USB) {
		value &= ~STARTON_USB;
		twl4030_i2c_write_u8(TWL4030_CHIP_PM_MASTER, value, PM_MASTER_CFG_P1_TRANSITION);
		twl4030_i2c_read_u8(TWL4030_CHIP_PM_MASTER, &value, PM_MASTER_CFG_P1_TRANSITION);
		printf("PM_MASTER_CFG_P1_TRANSITION=0x%x\n", value);
	}	
	
	twl4030_i2c_read_u8(TWL4030_CHIP_PM_MASTER, &value, PM_MASTER_CFG_P2_TRANSITION);

	if (value & STARTON_USB) {
		value &= ~STARTON_USB;
		twl4030_i2c_write_u8(TWL4030_CHIP_PM_MASTER, value, PM_MASTER_CFG_P2_TRANSITION);
		twl4030_i2c_read_u8(TWL4030_CHIP_PM_MASTER, &value, PM_MASTER_CFG_P2_TRANSITION);
		printf("PM_MASTER_CFG_P2_TRANSITION=0x%x\n", value);		
	}	

	twl4030_i2c_read_u8(TWL4030_CHIP_PM_MASTER, &value, PM_MASTER_CFG_P3_TRANSITION);


	if (value & STARTON_USB) {
		value &= ~STARTON_USB;
		twl4030_i2c_write_u8(TWL4030_CHIP_PM_MASTER, value, PM_MASTER_CFG_P3_TRANSITION);
		twl4030_i2c_read_u8(TWL4030_CHIP_PM_MASTER, &value, PM_MASTER_CFG_P3_TRANSITION);
		printf("PM_MASTER_CFG_P3_TRANSITION=0x%x\n", value);		
	}		
